...
const Banner = () => {
  return (
    <div className="relative pt-8 w-full flex justify-center overflow-hidden">
      <div className="hidden lg:flex w-full justify-center items-center gap-5 relative">
        {/* Prev Slide */}
        <div className="min-w-[60vw] h-[300px] absolute transition-all duration-500 opacity-50 scale-90 left-[-40%] rounded-2xl overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center text-white text-center p-5 px-10">
            ...
            <div className="w-1/2 flex flex-col items-center gap-4">
              <div className="bg-[#FFFFFF1A] rounded-[80px] font-semibold text-xl px-4 py-1">
                {banners[2].title}
              </div>
              <div>
                {banners[2].text.map((line, index) => (
                  <h2 className="text-4xl font-extrabold" key={index}>
                    {line}
                  </h2>
                ))}
              </div>

              <button className="bg-[#FF0960] uppercase font-semibold text-xl px-20 py-3 rounded-[10px]">
            ...
            <div className="w-1/2 flex flex-col items-center">
              ...
              <div>
                {banners[1].text.map((line, index) => (
                  <h2 className="text-4xl font-extrabold pb-2" key={index}>
                    {line}
                  </h2>
                ))}
              </div>
